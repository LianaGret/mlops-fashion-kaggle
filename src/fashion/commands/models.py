...
class Models:
    ...
    def train(
        ...
        self._run_training(cfg, resume)

    def _run_training(self, cfg: OmegaConf, resume: Path | None = None) -> None:
    ...
    def infer(
        ...
        if checkpoint is None:
            checkpoint = self._find_best_checkpoint(model_type)
            if checkpoint is None:
        ...
        if is_visual:
            ...
            self._run_visual_inference(model, device, output_path, batch_size, sample_customers)
        else:
            from fashion.data.datamodule import FashionDataModule
    ...
    def export(
        ...
        print_header("Model Export", "Converting to ONNX")
        console.print()

        print_key_value("Checkpoint", str(checkpoint))
        print_key_value("Output", str(output))
        print_key_value("Opset version", str(opset_version))
        console.print()
        ...
        print_info("Loading model...")
        model = FashionRecommender.load_from_checkpoint(str(checkpoint))
        model.eval()

        print_info("Exporting to ONNX...")
    ...
    def _find_best_checkpoint(self, model_type: str) -> Path | None:
        if not CHECKPOINTS_DIR.exists():
            return None
    ...
    def _get_version_info(self) -> dict[str, str]:
        ...
        jj_change = self._run_command(["jj", "log", "-r", "@", "--no-graph", "-T", "change_id"])
        if jj_change:
            info["jj_change_id"] = jj_change

        jj_commit = self._run_command(["jj", "log", "-r", "@", "--no-graph", "-T", "commit_id"])
        if jj_commit:
